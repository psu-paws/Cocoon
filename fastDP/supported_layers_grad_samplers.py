...
def _compute_embeddingbag_grad_sample(layer: nn.Embedding, A: torch.Tensor, B: torch.Tensor, clipping_mode: str) -> None:
    """Compute per-sample gradient norm for EmbeddingBag.

    General case (multi-hot): norm = ‖B[i]‖ · sqrt(Σⱼ rⱼ²), where rⱼ = # times row j
    appears in sample i's bag. Use the loop below when multi-hot bags are expected:

        indices, offsets = A
        def bag_grad_norm(i):
            start = offsets[i]
            end = offsets[i+1] if i < offsets.size(0) - 1 else indices.size(0)
            _, freq = indices[start:end].unique(return_counts=True)
            return B[i].norm(2) * math.sqrt((freq**2).sum())
        layer.weight.norm_sample = torch.stack([bag_grad_norm(i) for i in range(offsets.size(0))])

    Criteo simplification: each sample accesses exactly one row per table (k=1), so norm = ‖B[i]‖.
    """
    layer.weight.norm_sample = B.norm(2, dim=1)
...
def _clip_embeddingbag_grad(layer: nn.Embedding, A: torch.Tensor, B: torch.Tensor, C) -> None:
    """Accumulate clipped per-sample gradients into the embedding weight gradient.

    General case (multi-hot): expand B so each index in the bag gets its sample's gradient:
        diff_bag = offsets[1:] - offsets[:-1]; diff_bag[-1] = indices.size(0) - offsets[-1]
        row_ids = torch.arange(offsets.size(0), device=offsets.device).repeat_interleave(diff_bag)
        grad_weight.index_add_(0, indices, B[row_ids])

    Criteo simplification: one index per sample per table, so B[i] maps directly to that row.
    """
    indices, offsets = A
    grad_weight = torch.zeros((len(layer.weight), B.size(1)), device=offsets.device)
    grad_weight.index_add_(0, indices, B)
    return grad_weight
...
_supported_layers_norm_sample_AND_clipping = {
    ...
    nn.EmbeddingBag: (_compute_embeddingbag_grad_sample, _clip_embeddingbag_grad),
