...
class ManualIndexSampler(Sampler):
...
class CriteoDataset(Dataset):
    def __init__(
        ...
        self.d_path = os.environ.get("CRITEO_DATA_DIR", "./data") + "/"
        # total_file = self.d_path + self.d_file + "_day_count.npz"
        total_file = self.d_path + "train_day_count.npz"
    ...
    def __getitem__(self, index):
        ...
        if self.max_ind_range > 0:
            return self.X_int[i], self.X_cat[i] % self.max_ind_range, self.y[i], i, self.which_order, self.split
        else:
            return self.X_int[i], self.X_cat[i], self.y[i], i, self.which_order, self.split
...
def collate_wrapper_criteo_offset(list_of_tuples):
    batches = len(list_of_tuples)
    X_int_np = np.empty((batches, 13), dtype=np.float32)
    X_cat_np = np.empty((batches, 26), dtype=np.int64)
    T_np     = np.zeros((batches,),    dtype=np.float32)

    for i, (xint_i, xcat_i, t_i, _, _, _) in enumerate(list_of_tuples):
        X_int_np[i] = xint_i
        X_cat_np[i] = xcat_i
        T_np[i]     = t_i

    np.log1p(X_int_np, out=X_int_np)
    X_int = torch.as_tensor(X_int_np)         # (B, 13)
    X_cat = torch.as_tensor(X_cat_np)         # (B, 26)
    T     = torch.as_tensor(T_np)[:, None]    # (B, 1)

    featureCnt = X_cat.size(1)
    lS_i = X_cat.t().contiguous()
    base_row = torch.arange(batches, dtype=torch.long)
    lS_o = base_row.unsqueeze(0).expand(featureCnt, -1).contiguous()
...
def make_criteo_data_and_loaders(args, offset_to_length_converter=False):
    ...
    num_full_batches = (len(train_data) // args.mini_batch_size) * args.mini_batch_size
    for _ in range(args.nepochs):
        shuffled_epoch = torch.randperm(len(train_data)).tolist()
        shuffled_epoch = shuffled_epoch[:num_full_batches]
        all_indices.extend(shuffled_epoch)
    print("Shuffled indices: ", len(all_indices))
    manual_sampler = ManualIndexSampler(all_indices)
...
class RandomDataset(Dataset):
    def __init__(
        ...
        self.n_acc_steps = batch_size // mini_batch_size # gradient accumulation steps
        ...
        self.n = self.mini_batch_size

        (self.X, self.lS_o, self.lS_i) = generate_dist_input_batch(
            self.m_den,
            self.ln_emb,
            self.n,
        ...
        self.T = generate_random_output_batch(self.n, self.num_targets, self.round_targets)
    ...
    def __getitem__(self, index):
        ...
        return (self.X, self.lS_o, self.lS_i, self.T)
...
def make_random_data_and_loader(
    ...
    if args.data_size <= 1:
        args.data_size = args.num_batches * args.batch_size
