...
class CorNoiseGenerator(Generic[Tensor]):
    ...
    def step(self, y_i: Tensor) -> Tensor:
        ...
        if self._buf is not None:
            b, _ = get_ops().get_shape(self._buf)
            start_i = 0 if self._bandwidth is None else self._step - b
            y_i = y_i - (self._matrix[self._step:self._step + 1, start_i:self._step]
                         @ self._buf)[0]
        x_i = y_i / self._matrix[self._step, self._step]
        x_i_copy = get_ops().clone(x_i)[None]
        if self._buf is None:
            self._buf = x_i_copy
        else:
            self._buf = get_ops().concatenate(
                [self._buf, x_i_copy], axis=0)
        if self._bandwidth is not None and len(
                self._buf) > self._bandwidth - 1:
...
class BandedMatrixFactorizationMechanism(GaussianMechanism):
    ...
    def add_noise(
        ...
        correlated_noise = self._solver.step(flattened_noise)
    ...
    def add_noise_torch(
        ...
        # Generate a flattened noise and correlate with noises from previous
        # iterations.
        num_dimensions = len(sum_of_grads)
        flattened_noise = self._generate_flattened_noise(
            num_dimensions, noise_stddev, seed)
        correlated_noise = self._solver.step(flattened_noise)

        data_with_noise = sum_of_grads + correlated_noise
